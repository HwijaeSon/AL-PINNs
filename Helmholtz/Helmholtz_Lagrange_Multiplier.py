...
def train(u_model, lbd, trainloader, bdry_data, val_test, optimizer, loss_f) :
    ...
    for i, (data,) in enumerate(trainloader) :
        ...
        loss = loss1 + (lbd*output_bdry.view(-1)).mean()
        loss.backward()
        lbd.grad *= -1
...
def main_function(model_name, lr, lbd_lr, EPOCH, device) :
    ...
    for t in tqdm(range(0, EPOCH)) :
        ...
        loss, loss1, loss2, val_err, test_err = train(u_model, lbd, trainloader=train_loader,\
...
if __name__ == "__main__":
    ...
    best_model, total_loss, val_errs, test_errs = main_function(args.model, args.lr, args.lbd_lr, args.EPOCH, device)
